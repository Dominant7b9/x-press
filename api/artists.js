...
artistsRouter.post('/', (req, res, next) => {
    ...
    const sql = "INSERT INTO Artist (name, date_of_birth, biography, is_currently_employed) VALUES ($name, $dateOfBirth, $biography, $isEmployed)";
    const values = {
        $name: name,
...
artistsRouter.put('/:artistId', (req, res, next) => {
    ...
    const isEmployed = req.body.artist.isCurrentlyEmployed === 0 ? 0 : 1;
    const sql = "UPDATE Artist SET name = $name, date_of_birth = $dateOfBirth, biography = $biography, is_currently_employed = $isEmployed WHERE Artist.id = $artistId";
    const values = {
        $name: name,
        $dateOfBirth: dateOfBirth,
        $biography: biography,
